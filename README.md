# Locomotion and Manipulation with Isaacgymenvs


### About this repository

This repository contains two tasks -- navigation and object manipulation -- using Unitree A1 Robot.
...
```bash
python train.py task=A1Base
```

This is a simple walking task.

To evoke the object manipulation task, use:

```bash
python train.py task=A1Dribbling
```
...
```bash
python train.py task=A1Base headless=True
```

When running headlessly, you 
can stop it early using Control-C in the command line window.

### Loading trained models // Checkpoints

Checkpoints are saved by default in the folder `runs/EXPERIMENT_NAME/nn` where `EXPERIMENT_NAME` 
defaults to the task name, but can also be overridden via the `experiment` argument.

To load a trained checkpoint and continue training, use the `checkpoint` argument:

```bash
python train.py task=<task> checkpoint=runs/<experiment>/nn/<experiment>.pth
```
...
```bash
python train.py task=<task> checkpoint=runs/<experiment>/nn/<experiment>.pth test=True num_envs=64
```

Note that If there are special characters such as `[` or `=` in the checkpoint names, 
you will need to escape them and put quotes around the string. For example,
`checkpoint="./runs/A1Base/nn/last_ep\=501rew\[5981.31\].pth"`
